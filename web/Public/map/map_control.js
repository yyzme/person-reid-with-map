function mission(cameraid,time,xx,yy,cameralist,clen){
	...
	$.ajax({
		...
		async:true,
		beforeSend:function(){
			var loading="<div class=\"loading\"><img src=\"./static/icon/load.gif\" alt=\"\"/>loading...</div>";
			$(".videos").html(loading);
			$(".videos").show();
		},
		...
		success:function(data){
			...
			arrow = BM.polyline([ site[n-1],[xx,yy] ]).addTo(map);
			arrowHead = BM.polylineDecorator(arrow, {
				patterns: [
					{offset: '100%', repeat: 0, symbol: BM.Symbol.arrowHead({pixelSize: 15, polygon: false, pathOptions: {stroke: true}})}
				]
